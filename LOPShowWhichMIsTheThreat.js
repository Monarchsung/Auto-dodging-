                    ...
                    if (mob.speed) {
                        ...
                        if (dotProduct > 0) { // Missile is heading towards the player
                            ...
                            if (distance <= playerRadius) {
                                ...
                                // Draw short path away from the collision point
                                const awayPosX = mob.pos.x - mob.speed.x * 10; // Short path in the opposite direction
                                const awayPosY = mob.pos.y - mob.speed.y * 10;
                                const { screenX: awayScreenX, screenY: awayScreenY } = worldToScreen(awayPosX, awayPosY, playerPos.x, playerPos.y);
                                ctx.beginPath();
                                ctx.moveTo(screenX, screenY);
                                ctx.lineTo(awayScreenX, awayScreenY);
                                ctx.strokeStyle = 'rgba(0, 255, 0, 0.7)'; // Green short path
                                ctx.lineWidth = 2;
                                ctx.stroke();
                            } else {
                                if (showMissilePaths) {
